// Portions of this file are derived from gnark-crypto's batch-affine MSM:
// https://github.com/Consensys/gnark-crypto/blob/0a975d747c416958f20ad4e785bcf102449a81c9/ecc/bn254/multiexp_affine.go
//
// Copyright 2020-2026 Consensys Software Inc.
// Licensed under the Apache License, Version 2.0; see the repository's LICENSE-APACHE file.
// Ported to Rust, generalized over arkworks curves, and otherwise modified by TACEO.
...
//!   size, which performs poorly when several MSMs run concurrently (a Groth16 prover runs
//!   five). We only parallelize over windows.
...
//!
//! While the bucket method itself works for any group, the batch-affine bucket accumulation is
//! specialized to short-Weierstrass geometry: the batched addition is the SW chord formula
//! (see `BucketAccumulator::execute_batch`), digit signs are applied through the SW negation
//! `(x, y) -> (x, -y)`, the chord's exceptional cases (doubling and `P + (-P)`) are detected by
//! comparing coordinates, and the point at infinity, which has no affine coordinates, marks an
//! empty bucket. This is also why everything is generic over [`SWCurveConfig`] rather than
//! [`ark_ec::CurveGroup`]: the accumulator constructs points from the coordinates it computes,
//! and [`ark_ec::AffineRepr`] exposes no such constructor. Curves that arkworks models in
//! another form (e.g. twisted Edwards) would need their own window sum built on their own
//! addition formulas.
//!
//! Generic code that only knows a [`Pairing`](ark_ec::pairing::Pairing) can name the configs
//! with an equality bound:
//!
//! ```
//! use ark_ec::pairing::Pairing;
//! use ark_ec::short_weierstrass::{Affine, Projective, SWCurveConfig};
//!
//! fn msm_g1<P, C>(bases: &[P::G1Affine], scalars: &[P::ScalarField]) -> P::G1
//! where
...
//! }
//! # let _ = msm_g1::<ark_bn254::Bn254, _>(&[], &[]);
//! ```
...
/// Arkworks' extended-Jacobian bucket representation of the curve.
type BucketOf<P> = <Projective<P> as VariableBaseMSM>::Bucket;
type BigIntOf<P> = <<P as ark_ec::CurveConfig>::ScalarField as PrimeField>::BigInt;
...
pub fn msm_unchecked<P: SWCurveConfig>(
    bases: &[Affine<P>],
...
/// If the elements have different length, it will chop the slices to the
/// shortest length between `scalars.len()` and `bases.len()`.
pub fn msm_bigint<P: SWCurveConfig>(bases: &[Affine<P>], scalars: &[BigIntOf<P>]) -> Projective<P> {
    msm_windows(bases, scalars, |bases, digits, window_size, num_buckets| {
        if window_size >= MIN_WINDOW_FOR_BATCH_AFFINE {
            batch_affine_window_sum(bases, digits, num_buckets)
        } else {
            extended_jacobian_window_sum(bases, digits, num_buckets)
        }
    })
}
...
fn msm_windows<P: SWCurveConfig>(
    bases: &[Affine<P>],
    scalars: &[BigIntOf<P>],
    window_sum: impl Fn(&[Affine<P>], &[u32], usize, usize) -> Projective<P> + Send + Sync,
) -> Projective<P> {
    ...
    let digits = window_major_digits::<P>(scalars, window_size, num_bits, num_windows);
...
fn window_major_digits<P: SWCurveConfig>(
    scalars: &[BigIntOf<P>],
    window_size: usize,
    num_bits: usize,
    num_windows: usize,
...
fn batch_affine_window_sum<P: SWCurveConfig>(
    bases: &[Affine<P>],
    digits: &[u32],
    num_buckets: usize,
) -> Projective<P> {
    let mut accumulator = BucketAccumulator::new(num_buckets);
...
fn extended_jacobian_window_sum<P: SWCurveConfig>(
    bases: &[Affine<P>],
    digits: &[u32],
    num_buckets: usize,
) -> Projective<P> {
    let mut buckets = vec![Projective::<P>::ZERO_BUCKET; num_buckets];
    for (digit, base) in digits.iter().zip(bases) {
        if *digit == 0 {
            continue;
        }
...
impl<P: SWCurveConfig> BucketAccumulator<P> {
    ...
    /// pending batch.
    fn add_to_batch(&mut self, bucket: usize, x: P::BaseField, y: P::BaseField) {
        let Some((bucket_x, bucket_y)) = self.buckets[bucket].xy() else {
            self.buckets[bucket] = Affine::new_unchecked(x, y);
            return;
        };
        if bucket_x == x {
            if bucket_y == y {
...
mod tests {
    ...
    fn matches_arkworks_msm() {
        ...
        for size in [0, 1, 31, 32, 213, 810, 4096, 5000, 20000] {
            ...
            let actual = msm_unchecked(&bases, &scalars);
